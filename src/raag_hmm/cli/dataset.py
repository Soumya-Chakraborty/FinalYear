...
def extract_pitch_command(
    ...
    try:
        ...
        result = {
            ...
            "f0_hz": to_json_serializable(f0_hz),
            "voicing_prob": to_json_serializable(voicing_prob),
            "smoothed": smooth
        }
        ...
        if quantize:
            ...
            with console.status("[bold blue]Quantizing pitch..."):
                ...
                result["quantized"] = to_json_serializable(quantized)
        ...
        if output_file:
            ...
            # Convert to JSON string first to handle any remaining numpy types
            try:
                json_str = json.dumps(result, indent=2)
                console.print(json_str)
            except TypeError as e:
                console.print(f"[yellow]Warning: Could not display JSON due to serialization issue: {e}[/yellow]")
                console.print(f"[green]Pitch extraction completed successfully[/green]")
